...
bool* boolArrayOfNumbers(int const* array, int const length, int const* randomNumbers, int const lengthOfIntegers)
{
	bool* isOrNot = (bool*)malloc(sizeof(bool) * lengthOfIntegers);

	for (int i = 0; i < lengthOfIntegers; ++i)
	{
		isOrNot[i] = binarySearch(array, length, randomNumbers[i]);
	}
...
void existElementsInArray()
{
	...
	int* randomNumbers = (int*)malloc(lengthOfIntegers * sizeof(int));
	...
	bool* isOrNot = boolArrayOfNumbers(array, length, randomNumbers, lengthOfIntegers);
